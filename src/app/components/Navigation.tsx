...
export function Navigation() {
  ...
  const handleLogout = async () => {
    try {
      ...
      // IMPORTANT: Setăm user pe null înainte de signOut pentru a preveni erorile de permisiuni în ascultători
      setUser(null); 
      await signOut(auth);
      navigate("/");
    } catch (error) {
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center gap-2">
            ...
            {user && (
              <Link 
                to="/new-trip" 
                className="bg-blue-600 text-white px-5 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-[0.2em] flex items-center gap-2 hover:bg-blue-700 transition-all shadow-lg shadow-blue-600/20 mr-4"
              >
            ...
            <div className="flex items-center gap-3 pl-6 border-l border-gray-100 dark:border-gray-800">
              ...
              <div className="relative" ref={dropdownRef}>
                <button 
                  ...
                  className="flex items-center gap-2 p-1 rounded-2xl bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800 hover:border-blue-500/50 transition-all group pr-3"
                >
                  <div className="w-8 h-8 rounded-xl bg-blue-600 flex items-center justify-center text-white shadow-md overflow-hidden">
                    {user?.photoURL ? (
                ...
                {/* DROPDOWN MENU */}
                {isDropdownOpen && (
                  <div className="absolute right-0 mt-3 w-64 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-[2rem] shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                    {!user ? (
                      ...
                      <div className="p-2 flex flex-col gap-1">
                        {/* HEADER DROPDOWN ACTUALIZAT CU INFO USER */}
                        <div className="px-3 py-4 mb-1 flex items-center gap-3 rounded-t-[1.5rem]">
                          <div className="w-10 h-10 rounded-xl bg-blue-600 flex-shrink-0 flex items-center justify-center text-white shadow-sm overflow-hidden border-2 border-white dark:border-gray-800">
                            {user.photoURL ? (
                          ...
                          <div className="min-w-0">
                            <p className="text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] leading-none mb-1">Conectat ca</p>
                            <p className="text-xs font-black text-gray-900 dark:text-white truncate">
                                {user.displayName || "Explorator TravelWise"}
                            </p>
                            <p className="text-[10px] text-gray-500 truncate leading-none mt-1">{user.email}</p>
                          </div>
                        </div>
